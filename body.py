...
def update_status(user_id, role="employee"):
    ...
    try:
        complaint_id = input(" status open, on progress, resolved , closed \n"
                             "(You can only update your own complaints)Complaint ID: ").strip()
...
def add_receipt(user_id):
    ...
    receipt_number = int(input("Receipt number: ").strip())
    ...
    """
    db_cursor.execute(sql, (receipt_number,))
    result = db_cursor.fetchone()
